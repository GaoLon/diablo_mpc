...
#ifdef _WITH_ROS    
    #include <ros/ros.h>
    //#include "DIABLO_OSDK_msgs/Movement_Ctrl.h"
#endif
...
class Movement_Ctrl{
public:
    Movement_Ctrl(Vehicle* vehicle): vehicle(vehicle), ctrl_status(CTRL_RELEASED), dropCtrlCnt(0)
    {
        //memset(&ctrl_mode_data, 0, sizeof(OSDK_Movement_Ctrl_Mode_t));
        memset(&ctrl_data, 0, sizeof(OSDK_Movement_Ctrl_t));
        ctrl_mode_cmd = false;
        ctrl_mode_data.head_controller_mode=1;
        ctrl_mode_data.height_ctrl_mode=1;
        ctrl_mode_data.pitch_ctrl_mode=1;
        ctrl_mode_data.split_ctrl_mode=1;
        ctrl_mode_data.roll_ctrl_mode=1;
        ctrl_mode_data.yaw_ctrl_mode=0;
    ...
    {
        if(ctrl_mode != OSDK_CTRL_MODE_MOTION)
        {
            if(dropCtrlCnt > 10)
            {
